...
1. \$schema - JSON Schema 验证

作用：提供 JSON Schema 验证，在支持 Schema 的编辑器中获得智能提示和验证
推荐值：始终使用这个 URL
编辑器支持：VSCode、WebStorm 等都会自动识别并提供代码补全
验证功能：防止配置错误，比如拼写错误、类型错误等

2. collection - 代码生成器集合

作用：指定使用的代码生成器集合
默认值："@nestjs/schematics"（官方提供）
可选值："@nestjs/schematics" - 官方标准集合
自定义集合路径 - 如 "./my-schematics"
自定义用法：

```bash
# 使用自定义schematics
nest generate component my-component --collection=./my-schematics
```

3. sourceRoot - 源码根目录

作用：指定 TypeScript 源码的根目录
默认值："src"
修改场景：
多项目结构时可能需要调整
与其他框架集成时可能需要改变目录结构
注意事项：修改后需要相应调整 TypeScript 配置

4. monorepo - 多项目模式

- 作用：启用或禁用 monorepo（多项目）模式
- 默认值：false
- 启用效果：
  - 允许在projects中定义多个项目
  - 支持应用和库的混合开发
