...
所有值得重构的 Vue 项目，几乎都有自己的复杂度中心。前面 6 个案例可以很清楚地看到这件事：
...
前面 6 篇案例有一个共同点：所有真正有效的重构，都是先稳定模型，再拆视图。
...
前面 6 篇案例分别解决了不同场景的问题，但串起来以后，会发现它们最终都在做同一件事：把“混在组件里的复杂度”，重新分配到模型、状态中心、协议和组件边界上。
